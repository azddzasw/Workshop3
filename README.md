...
**Dataset Selection:** Our database is breast_cancer in sklearn,

**Model Development:** the train code is store in AI_mode_training.ipynb and model parameters is model.pkl

**API Development:**After model training, use flask to build an app for api  (model—api.py)
...
**Inter-computer Connectivity:** Connecting to the network using ngrok and  our data results are boolean values, we select the most frequently occurring value as the final result.
...
- Set up slashing, penalty functions, and establish consensus mechanisms and the model information is stored in model_info.json.
